  ...
  return (
    <div className="w-full max-w-sm rounded-3xl border bg-background p-6 shadow-xl sm:p-8">
      <div className="mb-8 flex flex-col items-center text-center">
        ...
        <p className="text-sm text-muted-foreground">Ingresa tu nombre para acceder al sistema</p>
      </div>
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        <div className="flex flex-col gap-2">
          <Label htmlFor="nombre">Nombre del Colaborador</Label>
          <Input id="nombre" type="text" placeholder="Ej: Rodrigo" value={nombre}
            onChange={(e) => setNombre(e.target.value)} disabled={isPending}
            className="h-12 rounded-xl text-base" autoComplete="off" />
          {error && <p className="text-sm font-medium text-destructive" role="alert" aria-live="polite">{error}</p>}
        </div>
        <Button type="submit" disabled={isPending} className="h-14 w-full rounded-2xl text-base font-semibold">
          {isPending ? "Ingresando..." : <><LogInIcon className="mr-2 size-5" /> Ingresar al Sistema</>}
        </Button>
      </form>
    </div>
  )
